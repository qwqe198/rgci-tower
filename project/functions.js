var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	exp *= -1;
	exp *= (core.getFlag('zjyzy'));
	exp = Math.floor(exp)
	core.status.hero.exp += exp;
	core.status.hero.statistics.exp += exp;


	var hint = "割" + core.getEnemyValue(enemy, "name", x, y);
	if (core.flags.statusBarItems.indexOf('enableMoney') >= 0)
		hint += ',' + core.getStatusLabel('money') + '+' + core.formatBigNumber(Math.ceil(core.getFlag('zczy'))); // hint += "，金币+" + money;
	if (core.flags.statusBarItems.indexOf('enableExp') >= 0)
		hint += ',剩余' + core.getStatusLabel('exp') + core.formatBigNumber(exp); // hint += "，经验+" + exp;
	...
	*/
	...
	return [
		...
		[22, "灵体", function (enemy) { return "敌人对角色造成" + (enemy.damage || 0) + "点固定伤害，该伤害可被护盾减免，减免效果为护盾的5倍。"; }, "#ff9977"],
		...
		[62, "自爆", "强者在绝望之下最后的尊严。\n第400回合触发，立即死亡，对角色造成自身剩余生命*5的伤害。", "#597a80"],
		...
		[78, "[∞]生灭", "生生不息，永恒不灭。\n每多一次维度提升，自身生命提升1.2倍，攻击、防御提升1倍。", "#82e065"],
		[79, "[∞]剑锋", "斩断长空，势如破竹。\n每多一次维度提升，自身攻击提升1.2倍，生命、防御提升1倍。", "#f5476a"],
		[80, "[∞]守御", "沉静的守护者。\n每多一次维度提升，自身防御提升1.2倍，生命、攻击提升1倍。", "#47b5f5"],
		[81, "[∞]无忌", function (enemy) { return "百无禁忌，是为王道。\n每多一次维度提升，生命增加（" + (enemy.value || 0) + "*维度提升次数）点，攻防增加（" + (enemy.atkValue || 0) + "*维度提升次数）点。"; }, "#C0C0C0"],
		[82, "[∞]无忌", function (enemy) { return "百无禁忌，是为王道。\n每多一次维度提升，生命增加（" + (enemy.value || 0) + "*维度提升次数）点，攻击增加（" + (enemy.atkValue || 0) + "*维度提升次数）点，防御增加（" + (enemy.defValue || 0) + "*维度提升次数）点。"; }, "#C0C0C0"],
		[83, "[∞]命运", "由内而外成为更高层次生命。\n每多一次维度提升，自身三围属性提升1.2倍。", "#db4f80"],
	];
},
	...
	if (core.hasSpecial(mon_special, 78)) {
		mon_hp = Math.floor(mon_hp * Math.pow((2.2 * core.getFlag('un1') * core.getFlag('un2')), (core.itemCount("I729"))));
		mon_atk = Math.floor(mon_atk * Math.pow((2 * core.getFlag('un1') * core.getFlag('un2')), (core.itemCount("I729"))));
		mon_def = Math.floor(mon_def * Math.pow((2 * core.getFlag('un1') * core.getFlag('un2')), (core.itemCount("I729"))));
	}
	// 攻击2
	...
	if (core.hasSpecial(mon_special, 79)) {
		mon_hp = Math.floor(mon_hp * Math.pow((2 * core.getFlag('un1') * core.getFlag('un2')), (core.itemCount("I729"))));
		mon_atk = Math.floor(mon_atk * Math.pow((2.2 * core.getFlag('un1') * core.getFlag('un2')), (core.itemCount("I729"))));
		mon_def = Math.floor(mon_def * Math.pow((2 * core.getFlag('un1') * core.getFlag('un2')), (core.itemCount("I729"))));
	}
	// 防御2
	...
	if (core.hasSpecial(mon_special, 80)) {
		mon_hp = Math.floor(mon_hp * Math.pow((2 * core.getFlag('un1') * core.getFlag('un2')), (core.itemCount("I729"))));
		mon_atk = Math.floor(mon_atk * Math.pow((2 * core.getFlag('un1') * core.getFlag('un2')), (core.itemCount("I729"))));
		mon_def = Math.floor(mon_def * Math.pow((2.2 * core.getFlag('un1') * core.getFlag('un2')), (core.itemCount("I729"))));
	}
	...
	// 固定3
	if (core.hasSpecial(mon_special, 81)) {
		mon_hp += (enemy.value || 0) * core.itemCount("I729") * core.itemCount("I729");
		mon_atk += (enemy.atkValue || 0) * core.itemCount("I729") * core.itemCount("I729");
		mon_def += (enemy.atkValue || 0) * core.itemCount("I729") * core.itemCount("I729");
	}
	...
	if (core.hasSpecial(mon_special, 82)) {
		mon_hp += (enemy.value || 0) * core.itemCount("I729") * core.itemCount("I729");
		mon_atk += (enemy.atkValue || 0) * core.itemCount("I729") * core.itemCount("I729");
		mon_def += (enemy.defValue || 0) * core.itemCount("I729") * core.itemCount("I729");
	}
	...
	//无限递增
	if (core.hasFlag('inft')) {
		var infenemy = core.getFlag('inft');
		if (infenemy < 5) {
			mon_hp *= 1 + ((core.getFlag('inft') - 1) * 0.16);
			mon_atk *= 1 + ((core.getFlag('inft') - 1) * 0.16);
			mon_def *= 1 + ((core.getFlag('inft') - 1) * 0.16);
		} else if (infenemy < 10) {
			mon_hp *= 1 + ((core.getFlag('inft') - 1) * 0.25);
			mon_atk *= 1 + ((core.getFlag('inft') - 1) * 0.25);
			mon_def *= 1 + ((core.getFlag('inft') - 1) * 0.25);
		} else {
			mon_hp *= 1 + ((core.getFlag('inft') - 1) * 0.36);
			mon_atk *= 1 + ((core.getFlag('inft') - 1) * 0.36);
			mon_def *= 1 + ((core.getFlag('inft') - 1) * 0.36);
		}
	}
	// 坚固
	...
	//星系
	hero_atk *= 1 + (core.itemCount("I890") / 1000);
	hero_def *= 1 + (core.itemCount("I890") / 1000);
	hero_mdef *= 1 + (core.itemCount("I890") / 1000);

	//无限护盾	
	if (core.hasItem("I923")) {
		var infmdef = 1 + (core.getFlag('ip', 0) / 100);
		if (infmdef > 922) { infmdef = 922; }
		hero_mdef *= infmdef;
	}
	...
	*/
		...
		try {
			...
			if (m481.indexOf(core.status.floorId) != -1) {
				...
				if (Date.now() - lastTime > 60) {
					...
					var image = core.material.images.images['m5.jpg']; // 获得图片，这里写图片名
	...
	if (!core.domStyle.isVertical) {
		...
		if (core.getFlag('jrsjtz1', 0) >= 1 || core.getFlag('jrsjtz2', 0) >= 1 || core.getFlag('jrsjtz3', 0) >= 1) {
			_fillBoldTextWithFontCheck(('水晶挑战中'), 75, 290, '#8d00ffff');
		}
		if (core.getFlag('jrcctz1', 0) >= 1 || core.getFlag('jrcctz2', 0) >= 1 || core.getFlag('jrcctz3', 0) >= 1) {
			_fillBoldTextWithFontCheck(('草场挑战中'), 75, 290, '#c3e4e6ff');
		}

		// 绘制生命
		//core.drawImage(ctx, core.statusBar.icons.hp, 15, 64, 20, 20);
		_fillBoldTextWithFontCheck(('等级:' + core.formatBigNumber(core.getRealStatus('hp')) + "(" + core.formatBigNumber(core.getRealStatus('exp')) + ")"), 65, 140, '#FFFFFF');

		// 绘制攻击
		//core.drawImage(ctx, core.statusBar.icons.atk, 3, 94, 16, 16);
		if (core.getFlag('jd', 0) >= 2) { _fillBoldTextWithFontCheck(('阶层:' + core.formatBigNumber((core.getFlag('jc', 0))) + "(" + core.formatBigNumber((core.getFlag('jcjy', 0))) + ")"), 60, 165, '#FFEF00'); } else { _fillBoldTextWithFontCheck(('未解锁'), 60, 165, '#FFEF00') }

		// 绘制防御
		//core.drawImage(ctx, core.statusBar.icons.def, 3, 122, 16, 16);
		_fillBoldTextWithFontCheck(('声望:'), 20, 190, '#1069f5');
		...
		_fillBoldTextWithFontCheck(('水晶:'), 20, 215, '#9810f5');
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('mdef')), 78, 215, "#9810f5");
		...
		if (core.hasItem('pack')) {
			_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('money')), 40, 91, "#F0E68C");
			_fillBoldTextWithFontCheck(core.getFlag('ip', 0), 40, 67, "#F0E68C");
			_fillBoldTextWithFontCheck(('无限点数'), 96, 67, '#f7cc76');
			_fillBoldTextWithFontCheck(('草'), 96, 91, '##8FF00');
			_fillBoldTextWithFontCheck(core.itemCount('I848'), 40, 115, '#FFCCAA');
		...
		if (core.getFlag('jrsjtz1', 0) >= 1 || core.getFlag('jrsjtz2', 0) >= 1 || core.getFlag('jrsjtz3', 0) >= 1) {
			_fillBoldTextWithFontCheck(('水晶挑战中'), 155, 127, '#8d00ffff');
		}
		if (core.getFlag('jrcctz1', 0) >= 1 || core.getFlag('jrcctz2', 0) >= 1 || core.getFlag('jrcctz3', 0) >= 1) {
			_fillBoldTextWithFontCheck(('草场挑战中'), 155, 127, '#c3e4e6ff');
		}
		// 绘制生命
		//core.drawImage(ctx, core.statusBar.icons.hp, 15, 64, 20, 20);
		_fillBoldTextWithFontCheck(('等级:' + core.formatBigNumber(core.getRealStatus('hp')) + "(" + core.formatBigNumber(core.getRealStatus('exp')) + ")"), 245, 45, '#FFFFFF');

		// 绘制攻击
		//core.drawImage(ctx, core.statusBar.icons.atk, 3, 94, 16, 16);
		if (core.getFlag('jd', 0) >= 2) { _fillBoldTextWithFontCheck(('阶层:' + core.formatBigNumber((core.getFlag('jc', 0))) + "(" + core.formatBigNumber((core.getFlag('jcjy', 0))) + ")"), 245, 72, '#FFEF00'); } else { _fillBoldTextWithFontCheck(('未解锁'), 245, 72, '#FFEF00') }

		// 绘制防御
		//core.drawImage(ctx, core.statusBar.icons.def, 3, 122, 16, 16);
		_fillBoldTextWithFontCheck(('声望:'), 214, 99, '#1069f5');
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('def')), 258, 99, "#5370e6");

		// 绘制护盾
		//core.drawImage(ctx, core.statusBar.icons.mdef, 0, 147, 22, 22);
		_fillBoldTextWithFontCheck(('水晶:'), 214, 127, '#9810f5');
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('mdef')), 258, 127, "#9810f5");
		...
		if (core.hasItem('pack')) {
			...
			_fillBoldTextWithFontCheck(core.getFlag('ip', 0), 34, 71, "#F0E68C");
			_fillBoldTextWithFontCheck(('无限点数'), 88, 71, '#f7cc76');
		...
		_fillBoldTextWithFontCheck(('白金'), 148, 25, '#778899');
		_fillBoldTextWithFontCheck((core.formatBigNumber(core.getRealStatus('mana'))), 148, 48, "#A9A9A9");
